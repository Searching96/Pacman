...
void drawTile(SDL_Renderer* rd, int x, int y, int type) {
    ...
    switch (type) {
    case 0: // Ô tr?ng
        SDL_SetRenderDrawColor(rd, 30, 30, 50, 255); // Màu xám t?i cho n?n
        break;
    case 1: // Ô t??ng
        SDL_SetRenderDrawColor(rd, 0, 0, 255, 255); // Màu xanh d??ng nh?t h?n
        break;
    default:
    ...
    // V? vi?n cho ô
    SDL_SetRenderDrawColor(rd, 200, 200, 200, 255); // Màu xám sáng h?n cho vi?n
    SDL_RenderDrawRect(rd, &tileRect);

    // N?u là ô t??ng, thêm hi?u ?ng vi?n sáng nh?
    if (type == 1) {
        SDL_SetRenderDrawColor(rd, 150, 150, 255, 255); // Màu xanh d??ng nh?t h?n cho vi?n sáng
        SDL_RenderDrawRect(rd, &tileRect); // Vi?n bên ngoài
        tileRect.x += 1;
        tileRect.y += 1;
        tileRect.w -= 2;
        tileRect.h -= 2;
        SDL_RenderDrawRect(rd, &tileRect); // Vi?n bên trong
        tileRect.x -= 1;
        tileRect.y -= 1;
        tileRect.w += 2;
        tileRect.h += 2;
