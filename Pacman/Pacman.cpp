...
void Pacman::render(SDL_Renderer* rd) {
    SDL_Color pacmanColor = { 255, 255, 0, 255 }; // Màu vàng cho thân Pacman

    int centerX = static_cast<int>(posX) + pacmanRect.w / 2;
    int centerY = static_cast<int>(posY) + pacmanRect.h / 2;
    int radius = pacmanRect.w / 2;

    float angle = atan2(-dirY, dirX) * (180.0f / M_PI); // Chuy?n ??i radian sang ??
    if (angle < 0) angle += 360; // ??m b?o góc trong kho?ng 0-360 ??

    // Thay ??i ?? m? mi?ng theo th?i gian
    Uint32 currentTime = SDL_GetTicks();
    if (currentTime - lastMouthChange > 400) { // Thay ??i m?i 300ms
        mouthState = (mouthState + 1) % 3; // Thay ??i gi?a 0, 1, và 2
        lastMouthChange = currentTime;
    }

    int mouthWidth = 45; // ?? r?ng mi?ng c? b?n
    mouthWidth = mouthWidth * (2 - mouthState) / 2; // ?i?u ch?nh ?? m? mi?ng

    int startAngle = static_cast<int>(angle - mouthWidth / 2);
    int endAngle = static_cast<int>(angle + mouthWidth / 2);

    // X? lý tr??ng h?p góc mi?ng v??t qua 360 ?? ho?c âm
    if (endAngle >= 360) {
