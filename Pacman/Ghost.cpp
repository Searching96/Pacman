...
void Ghost::render(SDL_Renderer* rd) {
    ...
    // Draw ghost's body (semi-circle)
    drawGhostBody(rd, static_cast<int>(posX), static_cast<int>(posY), ghostRect.w, ghostRect.h);

    // Draw ghost's eyes
    drawGhostEyes(rd, static_cast<int>(posX), static_cast<int>(posY), ghostRect.w, ghostRect.h);
